            ...
            <tr class="${
              g.referenceCode
                    ...
                    <td class="link"><a href="${
                      g.url
                    }" title="View on geocaching.com"><span class="glyphicon glyphicon-new-window"></span></a></td>
                </tr>`);
  });
...
$("#table-caches tbody").on("click", "input[type=checkbox]", function () {
  if ($("#chk_select").prop("checked") && $(this).prop("checked")) {
    ...
    $(
      "#table-caches tbody tr:nth-child(n+" +
        (countFrom + 1) +
        "):nth-child(-n+" +
        (countFrom + countTo) +
        ") input[type=checkbox]"
...
$("#chk_split").change(function () {
  $("#block_split input[type=range]").prop(
    "disabled",
    !$(this).prop("checked")
  );
});
...
$("#block_split input[type=range]").change(function () {
  $("label[for=chk_split]").html(
    "Split GPX files by " + $(this).val() + " geocaches"
  );
});

$("#chk_select").change(function () {
  $("#block_select input[type=range]").prop(
    "disabled",
    !$(this).prop("checked")
  );
});
...
$("#create-gpx").click(function () {
  ...
  let gpxSplit = $("#chk_split").prop("checked")
    ? +$("#block_split input[type=range]").val()
    : 0;
